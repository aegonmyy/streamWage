    ...
    <div className="mx-auto flex max-w-6xl flex-col px-4 py-16 sm:px-6">
      {!isConnected ? (
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <h1 className="mt-6 text-2xl font-semibold tracking-tight text-foreground">Welcome to StreamWage</h1>
          <p className="mt-2 max-w-md text-sm text-muted-foreground">
            Connect your wallet to discover payrolls you own, deploy a new one, or access a worker dashboard by contract address.
          </p>
          <div className="mt-10">
            <ConnectButton />
          </div>
        ...
        <div className="space-y-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Multi-tenant payroll</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground">Choose how you want to enter</h1>
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
              Open a payroll you own, deploy a new one, or enter as a worker with a contract address.
            </p>
          </div>
            ...
            <Card className="rounded-[28px] border-amber-200 bg-amber-50/70 shadow-sm">
              <CardHeader>
                ...
                <CardDescription className="text-amber-900/80">
                  This wallet has been nominated as the destination for a worker migration. Open the payroll below to review and accept it.
                </CardDescription>
              </CardHeader>
                ...
                {incomingMigrations.map((migration, index) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-amber-950">Accept worker migration</p>
          ...
          {payrolls.length > 0 ? (
            <Card className="rounded-[28px] border-border/70">
              <CardHeader>
                ...
                <CardDescription>
                  Contracts found from payroll deployment records, newest first.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <Card className="rounded-[28px] border-border/70">
              <CardHeader>
                <CardTitle>No payrolls deployed yet</CardTitle>
                <CardDescription>
                  This wallet does not currently own any deployed payroll contracts.
                </CardDescription>
              </CardHeader>
            </Card>
          )}
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="rounded-[28px] border-border/70">
              <CardHeader>
                ...
                <CardDescription>
                  Deploy a dedicated payroll contract on Hoodi for this wallet.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2 text-sm text-muted-foreground">
                  <p>Your own treasury</p>
                  <p>Your own workers</p>
                  <p>Full ownership</p>
                </div>
                <Button className="w-full rounded-xl" disabled={!factory || isWalletPending} onClick={() => void deployPayroll()}>
                  {isWalletPending ? "Deploying..." : "Deploy My Payroll"}
                </Button>
              </CardContent>
            </Card>
