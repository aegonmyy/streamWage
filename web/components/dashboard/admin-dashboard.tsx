...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    toast.error("Admin dashboard", {
      description: getDisplayErrorMessage(error, "Failed to load payroll."),
    })
  }, [error, isError])
    ...
    if (treasuryBalanceWei === 0n) {
      alerts.push(
        <AlertRow
          ...
          subText="All accrual is unfunded. Worker claims will fail."
          variant="danger"
          icon={OctagonAlert}
          action={
            <Button size="sm" className="rounded-xl h-9 px-5 text-sm" onClick={() => {
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-5">
          <MetricCard
            ...
            subLabel="Current treasury balance"
          />
          <MetricCard
            label="Daily drain"
            ...
            subLabel="Across all active time-based workers"
          />
          <MetricCard
            label="Runway"
            value={activeTimeBasedWorkers > 0 ? formatRunway(runwaySeconds) : "—"}
            subLabel={activeTimeBasedWorkers > 0 ? "At current burn rate" : "No active time-based workers"}
          ...
          <MetricCard
            ...
            subLabel={
              <div className="space-y-1">
                <p>{lowRunwayWorkers.length > 0 ? "Below threshold" : "All clear"}</p>
                {insolvencyCount > 0 && (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <QuickStat label="Workers" value={totalWorkersCount} />
            <QuickStat label="Proposals" value={pendingProposals.length} />
            <QuickStat label="Migrations" value={pendingMigrationsCount} />
  ...
  const renderTreasury = () => (
    <div className="space-y-6">
      <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
        <StatCard title="Balance" value={`${formatEth(treasuryBalanceWei)} ETH`} hint="Current balance." danger={isLowTreasury} />
        <StatCard title="Runway" value={formatDuration(runwaySeconds)} hint="From treasuryRunway()." />
        <StatCard title="Drain rate" value={formatWeiRatePerDay(totalRatePerSecondWei)} hint="Active payroll drain." />
        <StatCard title="Safe withdrawable" value={`${formatEth(safeWithdrawableWei)} ETH`} hint="Above the 1-hour reserve floor." />
      </div>
