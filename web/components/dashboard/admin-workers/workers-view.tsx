...
export function WorkersView() {
  ...
  const filteredWorkers = useMemo(() => {
    return workers.filter((worker) => {
      ...
      const matchesStatus = 
        ...
        (filter === "terminated" && worker.status === "terminated") ||
  ...
  async function executeWrite(actionLabel: string, callback: () => Promise<`0x${string}`>) {
    try {
      ...
      const message = getDisplayErrorMessage(caught, "Something went wrong.")
  ...
  return (
    <div className="space-y-10">
      ...
      <Dialog open={activeModal === "add"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Add a worker to this payroll.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={activeModal === "adjust-rate"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Change the pay rate for {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 pt-4">
      ...
      <Dialog open={activeModal === "propose-terms"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Propose new payroll terms for {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}. They'll need to accept or reject before the window closes.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 pt-4">
            <div className="space-y-2">
              <Label>Timeline</Label>
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl border border-destructive/20 bg-destructive/5">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-destructive/70 font-medium leading-tight">Worker is terminated if they reject or the proposal expires.</p>
              </div>
      ...
      <Dialog open={activeModal === "grant-payment"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Grant a one-off payment to {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 pt-4">
            <div className="p-4 rounded-2xl bg-muted/50 border border-border/40">
      ...
      <Dialog open={activeModal === "pause" || activeModal === "resume"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-sm rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {activeModal === "pause" 
                ...
                : `Resume accrual for ${selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}.`
              }
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 pt-4">
            ...
            >
              {activeModal === "pause" ? "Pause" : "Resume"}
            </Button>
      ...
      <Dialog open={activeModal === "cancel-proposal"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-sm rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Cancel the pending proposal for {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}?</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 pt-4">
            <Button
              variant="destructive"
            ...
            <Button variant="outline" className="h-11 rounded-xl" onClick={() => setActiveModal(null)}>Keep it</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={activeModal === "view-migration"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            ...
            <DialogDescription>Migration pending for {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 pt-4">
      ...
      <Dialog open={activeModal === "update-interval"} onOpenChange={(open) => !open && setActiveModal(null)}>
        <DialogContent className="max-w-md rounded-[28px]">
          <DialogHeader>
            <DialogTitle>Update</DialogTitle>
            <DialogDescription>Update the interval for {selectedWorker?.name || (selectedWorker?.address ? `${selectedWorker.address.slice(0,6)}...` : 'worker')}. All earned time is settled first.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5 pt-4">
            <div className="space-y-2">
              <Label>Timeline</Label>
            ...
            <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/10 flex gap-2 items-start">
              <Info className="h-4 w-4 text-amber-500 shrink-0 mt-0.5" />
              <p className="text-[11px] text-amber-600 font-medium">Takes effect immediately — no worker approval needed. Use Propose Terms if you want the worker to review first.</p>
            </div>
            <Button
              className="w-full h-11 rounded-xl font-bold"
              disabled={isWalletPending}
              onClick={() => executeWrite("Update interval", async () => {
                if (!selectedWorker) throw new Error("No worker selected")
                if (!contract) throw new Error("Contract not configured")
                return writeContractAsync({
                  ...contract,
                  functionName: "updateWorkerInterval",
                  args: [
                    selectedWorker.address,
                    Number(formTimeline),
                    formTimeline === "2" ? BigInt(formInterval || "0") : 0n,
        ...
        <DialogContent className="max-w-md rounded-[28px]">
          ...
          <div className="space-y-5 pt-4">
            ...
            <Button
              className="w-full h-11 rounded-xl font-bold"
              disabled={isWalletPending}
              onClick={() => executeWrite("Update metadata", async () => {
                if (!selectedWorker) throw new Error("No worker selected")
                if (!contract) throw new Error("Contract not configured")
                return writeContractAsync({
                  ...contract,
