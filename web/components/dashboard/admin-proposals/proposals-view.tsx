      ...
      <div className="space-y-10">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <ProposalStatCard
            ...
            value={stats.pending}
            subLabel="Waiting on worker"
            variant={stats.pending > 0 ? "warning" : "neutral"}
          />
          <ProposalStatCard
            label="Terminate on Reject"
            value={stats.terminateOnReject}
            subLabel="Worker is terminated on reject or expiry"
            variant={stats.terminateOnReject > 0 ? "danger" : "neutral"}
            tooltip="Worker is permanently deactivated if they reject or let this expire. Accrued balance stays claimable."
          />
          <ProposalStatCard
            label="Paused Workers"
            value={stats.pausedByProposal}
            subLabel="Accrual paused"
          ...
          <ProposalStatCard
            ...
            subLabel="Default window for new proposals"
            tooltip="Only applies to future proposals"
          />
        </div>
        ...
        <Dialog open={activeModal === "propose"} onOpenChange={(open) => !open && setActiveModal(null)}>
          <DialogContent className="max-w-md rounded-[28px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Worker must accept or reject before the window closes.
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={activeModal === "edit-window"} onOpenChange={(open) => !open && setActiveModal(null)}>
          <DialogContent className="max-w-sm rounded-[28px]">
            <DialogHeader>
              ...
              <DialogDescription>How long workers have to respond to new proposals.</DialogDescription>
            </DialogHeader>
            <div className="space-y-5 pt-4">
              <div className="space-y-2">
                <Label>Days</Label>
        ...
        <Dialog open={activeModal === "cancel"} onOpenChange={(open) => !open && setActiveModal(null)}>
          <DialogContent className="max-w-sm rounded-[28px]">
            <DialogHeader>
              ...
              <DialogDescription>
                The worker will be restored to their previous terms and accrual resumes.
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={activeModal === "resolve"} onOpenChange={(open) => !open && setActiveModal(null)}>
          <DialogContent className="max-w-sm rounded-[28px]">
            ...
            <div className="flex flex-col gap-3 pt-4">
              ...
              >
                {selectedWorker?.pendingProposal?.terminateOnReject ? "Terminate" : "Resolve"}
              </Button>
