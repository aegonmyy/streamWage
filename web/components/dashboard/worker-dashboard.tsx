...
export function WorkerDashboard() {
  ...
  useEffect(() => {
    ...
    toast.error("Worker dashboard", {
      description: getDisplayErrorMessage(error, "Failed to load worker data."),
    })
  }, [error, isError])
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          hint={isSolvent ? "Ready to claim." : `Shortfall: ${formatEth(shortfallWei)} ETH`}
          danger={!isSolvent}
        >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="cursor-help">
                <StatCard
                  title="Worker Runway"
                  value={data.runwaySeconds === 0n ? "—" : formatDuration(data.runwaySeconds)}
                  hint={data.runwaySeconds === 0n ? (data.active ? "Treasury empty or no active accrual." : "No active accrual — paused.") : "Estimated runway."}
  ...
  const renderEarnings = () => (
    <div className="space-y-4 sm:space-y-6">
      <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
        <StatCard title="Claimable now" value={`${formatEth(theoreticalWei)} ETH`} hint={isSolvent ? "Ready to claim." : `Treasury capped: ${formatEth(actualClaimableWei)} ETH`} danger={!isSolvent} />
        <StatCard title="Accrued checkpoint" value={`${formatEth(data.accruedWei)} ETH`} hint="Settled on-chain." />
        <StatCard title="Total claimed" value={`${formatEth(data.totalClaimedWei)} ETH`} hint="All time." />
        <StatCard title="Current rate" value={data.timeline === "Trigger" ? "Trigger" : formatRate(data)} hint="Your pay structure." />
      </div>
      ...
      <Card className={cn(data.pendingProposal && "border-warning/50 bg-warning/5", "rounded-[12px] md:rounded-2xl")}>
        <CardHeader className="p-4 md:p-6 pb-2 md:pb-2">
          ...
          <CardDescription className="text-xs md:text-sm">
            {data.pendingProposal ? "Accrual paused until this is resolved." : "Nothing pending."}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-4 md:p-6 pt-0 md:pt-0">
          {!data.pendingProposal ? (
            ...
            <div className="space-y-4">
              <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
                ...
                <StatCard title="Proposed rate" value={data.pendingProposal.timeline === "Trigger" ? "Trigger" : `${formatEth(data.pendingProposal.amountPerIntervalWei)} ETH`} hint="Per interval." />
                <StatCard title="Expires" value={humanExpiry(data.pendingProposal.expiryTimestamp)} hint="Either side can call expire." danger />
                <StatCard title="Reject effect" value={data.pendingProposal.terminateOnReject ? "Terminate" : "Resume old terms"} hint="Set by the operator." danger={data.pendingProposal.terminateOnReject} />
              </div>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        {data.pendingProposal.terminateOnReject
                          ? "Rejecting will permanently terminate your worker record."
                          : "Rejecting restores your previous terms."}
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Back</AlertDialogCancel>
