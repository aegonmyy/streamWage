...
export function usePayrollAdminData() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<PayrollAdminData> => {
      ...
      const terminated = new Set<Address>()
      ...
      for (const log of logs) {
        ...
        if (decoded.eventName === "WorkerTerminated") {
          terminated.add(getAddress(args.worker))
        }

        if (decoded.eventName === "ProposalExpired" && args.terminated === true) {
          terminated.add(getAddress(args.worker))
        }
      ...
      for (let index = 0; index < workerAddresses.length; index += 1) {
        ...
        const isTerminated = terminated.has(workerAddress) && !worker[1]
        ...
        workerRecords.push({
          ...
          pendingMigration: pendingMigration[0]
            ...
            : null,
          status: worker[1] ? "active" : isTerminated ? "terminated" : "paused",
        })
      }
